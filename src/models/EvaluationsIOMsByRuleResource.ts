/* tslint:disable */
/* eslint-disable */
/**
 * CrowdStrike API Specification
 * Use this API specification as a reference for the API endpoints you can use to interact with your Falcon environment. These endpoints support authentication via OAuth2 and interact with detections and network containment. For detailed usage guides and examples, see our [documentation inside the Falcon console](https://falcon.crowdstrike.com/support/documentation).     To use the APIs described below, combine the base URL with the path shown for each API endpoint. For commercial cloud customers, your base URL is `https://api.crowdstrike.com`.    Each API endpoint requires authorization via an OAuth2 token. Your first API request should retrieve an OAuth2 token using the `oauth2/token` endpoint, such as `https://api.crowdstrike.com/oauth2/token`. For subsequent requests, include the OAuth2 token in an HTTP authorization header. Tokens expire after 30 minutes, after which you should make a new token request to continue making API requests.
 *
 * The version of the OpenAPI document: rolling
 *
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from "../runtime";
import type { EvaluationsRule } from "./EvaluationsRule";
import { EvaluationsRuleFromJSON, EvaluationsRuleFromJSONTyped, EvaluationsRuleToJSON } from "./EvaluationsRule";
import type { DomainCloudGroup } from "./DomainCloudGroup";
import { DomainCloudGroupFromJSON, DomainCloudGroupFromJSONTyped, DomainCloudGroupToJSON } from "./DomainCloudGroup";
import type { EvaluationsCompliance } from "./EvaluationsCompliance";
import { EvaluationsComplianceFromJSON, EvaluationsComplianceFromJSONTyped, EvaluationsComplianceToJSON } from "./EvaluationsCompliance";

/**
 *
 * @export
 * @interface EvaluationsIOMsByRuleResource
 */
export interface EvaluationsIOMsByRuleResource {
    /**
     *
     * @type {string}
     * @memberof EvaluationsIOMsByRuleResource
     */
    accountId?: string;
    /**
     *
     * @type {number}
     * @memberof EvaluationsIOMsByRuleResource
     */
    assessedAssets: number;
    /**
     *
     * @type {string}
     * @memberof EvaluationsIOMsByRuleResource
     */
    cid: string;
    /**
     *
     * @type {Array<DomainCloudGroup>}
     * @memberof EvaluationsIOMsByRuleResource
     */
    cloudGroupsV2?: Array<DomainCloudGroup>;
    /**
     *
     * @type {string}
     * @memberof EvaluationsIOMsByRuleResource
     */
    cloudProvider: string;
    /**
     *
     * @type {Array<EvaluationsCompliance>}
     * @memberof EvaluationsIOMsByRuleResource
     */
    compliance: Array<EvaluationsCompliance>;
    /**
     *
     * @type {number}
     * @memberof EvaluationsIOMsByRuleResource
     */
    misconfigurations: number;
    /**
     *
     * @type {string}
     * @memberof EvaluationsIOMsByRuleResource
     */
    region: string;
    /**
     *
     * @type {EvaluationsRule}
     * @memberof EvaluationsIOMsByRuleResource
     */
    rule: EvaluationsRule;
    /**
     *
     * @type {string}
     * @memberof EvaluationsIOMsByRuleResource
     */
    severity: string;
    /**
     *
     * @type {{ [key: string]: string; }}
     * @memberof EvaluationsIOMsByRuleResource
     */
    tags: { [key: string]: string };
}

/**
 * Check if a given object implements the EvaluationsIOMsByRuleResource interface.
 */
export function instanceOfEvaluationsIOMsByRuleResource(value: object): value is EvaluationsIOMsByRuleResource {
    if (!("assessedAssets" in value) || value["assessedAssets"] === undefined) return false;
    if (!("cid" in value) || value["cid"] === undefined) return false;
    if (!("cloudProvider" in value) || value["cloudProvider"] === undefined) return false;
    if (!("compliance" in value) || value["compliance"] === undefined) return false;
    if (!("misconfigurations" in value) || value["misconfigurations"] === undefined) return false;
    if (!("region" in value) || value["region"] === undefined) return false;
    if (!("rule" in value) || value["rule"] === undefined) return false;
    if (!("severity" in value) || value["severity"] === undefined) return false;
    if (!("tags" in value) || value["tags"] === undefined) return false;
    return true;
}

export function EvaluationsIOMsByRuleResourceFromJSON(json: any): EvaluationsIOMsByRuleResource {
    return EvaluationsIOMsByRuleResourceFromJSONTyped(json, false);
}

export function EvaluationsIOMsByRuleResourceFromJSONTyped(json: any, ignoreDiscriminator: boolean): EvaluationsIOMsByRuleResource {
    if (json == null) {
        return json;
    }
    return {
        accountId: json["account_id"] == null ? undefined : json["account_id"],
        assessedAssets: json["assessed_assets"],
        cid: json["cid"],
        cloudGroupsV2: json["cloud_groups_v2"] == null ? undefined : (json["cloud_groups_v2"] as Array<any>).map(DomainCloudGroupFromJSON),
        cloudProvider: json["cloud_provider"],
        compliance: (json["compliance"] as Array<any>).map(EvaluationsComplianceFromJSON),
        misconfigurations: json["misconfigurations"],
        region: json["region"],
        rule: EvaluationsRuleFromJSON(json["rule"]),
        severity: json["severity"],
        tags: json["tags"],
    };
}

export function EvaluationsIOMsByRuleResourceToJSON(value?: EvaluationsIOMsByRuleResource | null): any {
    if (value == null) {
        return value;
    }
    return {
        account_id: value["accountId"],
        assessed_assets: value["assessedAssets"],
        cid: value["cid"],
        cloud_groups_v2: value["cloudGroupsV2"] == null ? undefined : (value["cloudGroupsV2"] as Array<any>).map(DomainCloudGroupToJSON),
        cloud_provider: value["cloudProvider"],
        compliance: (value["compliance"] as Array<any>).map(EvaluationsComplianceToJSON),
        misconfigurations: value["misconfigurations"],
        region: value["region"],
        rule: EvaluationsRuleToJSON(value["rule"]),
        severity: value["severity"],
        tags: value["tags"],
    };
}
