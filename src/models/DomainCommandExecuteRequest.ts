/* tslint:disable */
/* eslint-disable */
/**
 * CrowdStrike API Specification
 * Use this API specification as a reference for the API endpoints you can use to interact with your Falcon environment. These endpoints support authentication via OAuth2 and interact with detections and network containment. For detailed usage guides and examples, see our [documentation inside the Falcon console](https://falcon.crowdstrike.com/support/documentation).     To use the APIs described below, combine the base URL with the path shown for each API endpoint. For commercial cloud customers, your base URL is `https://api.crowdstrike.com`.    Each API endpoint requires authorization via an OAuth2 token. Your first API request should retrieve an OAuth2 token using the `oauth2/token` endpoint, such as `https://api.crowdstrike.com/oauth2/token`. For subsequent requests, include the OAuth2 token in an HTTP authorization header. Tokens expire after 30 minutes, after which you should make a new token request to continue making API requests.
 *
 * The version of the OpenAPI document: rolling
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
/**
 * 
 * @export
 * @interface DomainCommandExecuteRequest
 */
export interface DomainCommandExecuteRequest {
    /**
     * 
     * @type {string}
     * @memberof DomainCommandExecuteRequest
     */
    baseCommand: string;
    /**
     * 
     * @type {string}
     * @memberof DomainCommandExecuteRequest
     */
    commandString: string;
    /**
     * 
     * @type {string}
     * @memberof DomainCommandExecuteRequest
     */
    deviceId: string;
    /**
     * 
     * @type {number}
     * @memberof DomainCommandExecuteRequest
     */
    id: number;
    /**
     * 
     * @type {boolean}
     * @memberof DomainCommandExecuteRequest
     */
    persist: boolean;
    /**
     * 
     * @type {string}
     * @memberof DomainCommandExecuteRequest
     */
    sessionId: string;
}

/**
 * Check if a given object implements the DomainCommandExecuteRequest interface.
 */
export function instanceOfDomainCommandExecuteRequest(value: object): value is DomainCommandExecuteRequest {
    if (!('baseCommand' in value) || value['baseCommand'] === undefined) return false;
    if (!('commandString' in value) || value['commandString'] === undefined) return false;
    if (!('deviceId' in value) || value['deviceId'] === undefined) return false;
    if (!('id' in value) || value['id'] === undefined) return false;
    if (!('persist' in value) || value['persist'] === undefined) return false;
    if (!('sessionId' in value) || value['sessionId'] === undefined) return false;
    return true;
}

export function DomainCommandExecuteRequestFromJSON(json: any): DomainCommandExecuteRequest {
    return DomainCommandExecuteRequestFromJSONTyped(json, false);
}

export function DomainCommandExecuteRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): DomainCommandExecuteRequest {
    if (json == null) {
        return json;
    }
    return {
        
        'baseCommand': json['base_command'],
        'commandString': json['command_string'],
        'deviceId': json['device_id'],
        'id': json['id'],
        'persist': json['persist'],
        'sessionId': json['session_id'],
    };
}

export function DomainCommandExecuteRequestToJSON(value?: DomainCommandExecuteRequest | null): any {
    if (value == null) {
        return value;
    }
    return {
        
        'base_command': value['baseCommand'],
        'command_string': value['commandString'],
        'device_id': value['deviceId'],
        'id': value['id'],
        'persist': value['persist'],
        'session_id': value['sessionId'],
    };
}

