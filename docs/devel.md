...
- small portion of the files in the repo has been hand written by human (without assistance of AI or ML)
  ...
  - and `./src/client.ts` that is meant to represent an entry point class for end-users (the API imports, property declarations, and constructor assignments are regenerated by `scripts/generate_client.py` via `rebuild.sh`; the rest is hand-written)
